...
if [[ "$MAJOR_MINOR" < "24.3" ]]; then
  echo "Error: Version must be 24.3 or higher. LDR not supported below this version."
  exit 1
elif [[ "$MAJOR_MINOR" == "25.0" || "$MAJOR_MINOR" == "25" ]]; then
  echo "Note: Support for v25.x will be available in a future version of this script."
  exit 0
fi
